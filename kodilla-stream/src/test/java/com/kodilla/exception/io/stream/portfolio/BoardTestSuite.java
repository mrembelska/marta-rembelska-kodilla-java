package com.kodilla.exception.io.stream.portfolio;
...
public class BoardTestSuite {
    ...
    @Test
    public void testAddTaskListAverageWorkingOnTaskWithTwoStreams() {
        //given
        Board project = prepareTestData();
        ...
        long inProgressAllTasksDuration = project.getTaskLists().stream()
                .filter(inProgressTasks::contains)
                .flatMap(taskList -> taskList.getTasks().stream())
                .map(task -> task.getCreated())
                .mapToLong(dateOfCreation -> DAYS.between(dateOfCreation, LocalDate.now()))
        ...
        long inProgressTasksQuantity = project.getTaskLists().stream()
                .filter(inProgressTasks::contains)
                .flatMap(taskList -> taskList.getTasks().stream())
                .count();
